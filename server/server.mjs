...
async function askGwdg({message,contextPack}){
  const response=await fetch((process.env.GWDG_BASE_URL??'https://chat-ai.academiccloud.de/v1')+'/chat/completions',{
    ...
    body:JSON.stringify({
      ...
      messages:[
        {role:'system',content:'Du bist ein vorsichtiger Hof-Assistent. Trenne Beobachtung, mögliche Erklärungen, Prüfungen und nächste Schritte. Stelle agronomische Einschätzungen als Hinweise mit Unsicherheit dar und erfinde keine Quellen.'},
...
const server=createServer(async(request,response)=>{
  if(!allowedMethods.has(request.method??'')){response.writeHead(405);response.end();return}
  const url=new URL(request.url??'/',`http://${request.headers.host??'localhost'}`);
  try{
    if(url.pathname==='/health') return sendJson(response,200,{ok:true});
    if(url.pathname==='/api/assistant'&&request.method==='POST'){
      if(!process.env.GWDG_API_KEY||!process.env.GWDG_ARCANA_ID) return sendJson(response,503,{error:'GWDG server configuration is missing.'});
      const body=await readBody(request);
      if(typeof body.message!=='string'||!body.message.trim()) return sendJson(response,400,{error:'message is required.'});
...
server.listen(port,()=>console.log(`KlarblattFarm server listening on http://localhost:${port}`));